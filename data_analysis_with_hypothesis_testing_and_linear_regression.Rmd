...
author: "Alina Yildir, Paul Lin, Adam Virani, Vineet Patel, Utsav Goti, Dhruv Patel"
date: "2024-09-30"
output: pdf_document
---
...
Understanding life expectancy is pivotal in public health and socio-economic research for shaping policies and improving health outcomes. This study, "Exploring Life Expectancy: A Data Analysis Using Hypothesis Testing and Linear Regression," delves into the intricate relationships between various health indicators, socio-economic factors, and life expectancy across different countries. By employing rigorous statistical methods such as hypothesis testing and linear regression, the authors aim to uncover patterns and correlations that can inform policymakers and health professionals about the determinants of life expectancy.

The project's objectives:

**Exploratory Data Analysis**: To identify initial patterns, relationships, and insights within the dataset, setting the foundation for further analysis.

**GDP and Life Expectancy Analysis**: To examine the relationship between GDP and life expectancy using hypothesis testing and linear regression, determining the impact of economic factors on health outcomes.

**Schooling and Life Expectancy Analysis**: To investigate how years of schooling correlate with life expectancy through hypothesis testing and linear regression, highlighting the role of education in health.

**Developed vs. Developing Countries Analysis**: To compare life expectancy between developed and developing countries using hypothesis testing, identifying disparities and potential areas for policy intervention.

**Alcohol Consumption and Life Expectancy Analysis**: To explore the correlation between alcohol consumption and life expectancy through hypothesis testing, understanding its impact on health.

**Multiple Linear Regression Analysis**: To perform a comprehensive analysis using multiple linear regression, integrating various factors to predict life expectancy and identify critical determinants.

The analysis utilizes a comprehensive dataset (1) containing 2,938 observations and 22 variables, which include demographic, health, and economic indicators. Key variables analyzed include adult mortality rates, infant deaths, healthcare expenditure, vaccination rates, and socio-economic status. The study aims to provide insights into how these factors influence life expectancy, particularly in developing nations where health disparities are most pronounced.
...
## GDP and Life Expectancy: Hypothesis Testing and Linear Regression
...
Following observation can be done from the above visualization:
...
According to the Pearson correlation, a slightly positive association has been found between GDP and life expectancy. This indicates that life expectancy normally tends to increase along with GDP growth.
...
- Normality of residuals: The residuals' distribution with respect to a normal distribution is displayed in the Q-Q residual plot. The residuals appear to be roughly normally distributed because the majority of the spots are in close proximity to the theoretical line. 
...
Null Hypothesis ($H_{0}$): There is no relationship between schooling and life expectancy.

Alternative Hypothesis ($H_{1}$): More schooling leads to higher life expectancy.

### Data Loading and Preparation

```{r}
data1_filtered <- dataset %>%
  filter(Schooling != 0,
         !is.na(Schooling) & Schooling != "",
         `Life.expectancy` != 0,
         !is.na(`Life.expectancy`) & `Life.expectancy` != "")
```
...
The extremely low p-value allows us to reject the null hypothesis, confirming that there is a statistically significant relationship between schooling and life expectancy. Schooling has a meaningful impact on life expectancy.

When schooling is 0 years, the estimated life expectancy is 41.3 years. Additionally, for each additional year of schooling, life expectancy is expected to increase by approximately 2.32 years.

The R-squared value of 0.6139 indicates that 61.39% of the variation in life expectancy is explained by differences in schooling, highlighting the substantial influence of education on life expectancy.
...
In the residuals vs. fitted plot, most residuals cluster around the horizontal line at 0, indicating that the linearity assumption is met. The Q-Q plot shows the residuals closely following a straight line, confirming that the normality assumption holds. Furthermore, the residuals in the residuals vs. fitted plot are randomly scattered, suggesting that homoscedasticity is satisfied, with no discernible patterns in the spread.

### Hypothesis Testing: T-Test

```{r}
coef_summary <- summary(model)$coefficients
t_statistic <- coef_summary["Schooling", "Estimate"] /
  coef_summary["Schooling", "Std. Error"]
...
cat("95% Confidence Interval for the Schooling coefficient:",
    lower_bound, "to", upper_bound, "\n")
```

Very strong evidence is that for each year of schooling, life expectancy is expected to increase between 2.26 and 2.38 years.

### Discussion and Conclusion

There is a significant relationship between schooling and life expectancy, as shown by the very low p-value in the regression model. For every year of schooling, we expect life expectancy to increase by about 2.32 years, and about 61% of the variance in life expectancy can be explained by schooling.

## Developed vs. Developing Countries: Hypothesis Testing and Linear Regression
...
Null Hypothesis ($H_{0}$): There is no significant difference in life expectancy between developed and developing countries.

Alternative Hypothesis ($H_{1}$): There is a significant difference in life expectancy between developed and developing countries.
...
## Alcohol Consumption and Life Expectancy: Hypothesis Testing and Linear Regression
...
We can measure the strength and relationship between Alcohol Consumption and Life Expectancy.
...
Null Hypothesis ($H_{0}$): $r = 0$ There is no significant correlation between alcohol consumption and life expectancy.

Alternative Hypothesis ($H_{1}$): $r \neq 0$ There is a significant correlation between alcohol consumption and life expectancy.
...
After performing the Pearson correlation analysis, we obtained a correlation coefficient 0.405 with a p-value of 0.

The r value of 0.405 indicates a moderate positive correlation between Alcohol Consumption and Life Expectancy. This suggests that, as alcohol consumption increases, life expectancy tends to also increase, although this relationship is not very strong.
...
Mean Alcohol Consumption: 4.61 litres per capita.
...
ggplot(data, aes(x = Life_Expectancy_Group, y = Alcohol)) +
  ...
  labs(title = "Alcohol Consumption by Life Expectancy Group",
...
For the group with above median life expectancy, the median alcohol consumption is around 6 litres per capita. The box is more extended, suggesting that there is a broader spread of alcohol consumption within this group. Despite the variability, most of the values fall within a relatively moderate range of alcohol consumption, implying that in regions with higher life expectancy, alcohol consumption is generally moderate.

A few extreme outliers exist, particularly those consuming more than 15 litres per capita. These indicate that even in areas where life expectancy is above the median, some regions or countries have very high levels of alcohol consumption. However, the majority of these regions maintain consumption within a moderate range.

Below Median Life Expectancy:

In contrast, for the group with below median life expectancy, the median alcohol consumption is noticeably lower than that of the group with above median life expectancy. The box plot reveals that this group has more variation and a more significant number of outliers, with values extending beyond 10 litres per capita, showing a wider range of alcohol consumption in regions with lower life expectancy.

The presence of more outliers suggests that in areas with lower life expectancy, alcohol consumption levels vary significantly, with some regions exhibiting relatively high consumption, which may be associated with a lower life expectancy. This variation could reflect social, economic, or health factors influencing alcohol consumption and life expectancy.
...
From above parts of EDA, there are variables that exhibit potential nonlinear correlations with Y, specifically Life Expectancy. To address this, we introduce polynomial features and interaction features involving these variables.
...
### Multiple Linear Regression - Remove Non-Significant Variables

```{r}
data = subset(data, select = -c(thinness.5.9.years,thinness..1.19.years,
...
```{r}
predictions <- predict(model_2, newdata = data, interval = "prediction",
                       level = 0.95)
...
# Standardize
model_coefficients["Income.composition.of.resources"] <-
  model_coefficients["Income.composition.of.resources"]/100
...
We standardize the Income.composition.of.resources to percentage for better interpretation. 
...
\
\
-Income Composition of Resources (Estimate: 0.07485): This variable has the largest positive coefficient, suggesting that improvements in income composition of resources significantly enhance life expectancy. For every percentage increase in this index, life expectancy increases by approximately 0.07485 years, indicating the critical role of economic factors in health outcomes.
\
\
...
Our analysis reveals several key insights into the factors that influence life expectancy across countries:

- Variables such as Adult Mortality (-0.69), Infant Mortality (-0.33), Child Mortality (Under 5) (-0.36), and HIV/AIDS Child Mortality (Under 4) (-0.57) show strong to moderate negative correlations with life expectancy. These results highlight the significant adverse impact of mortality rates on life expectancy, particularly adult and child mortality.

- Thinness in younger populations (ages 5-19) also shows a strong negative correlation (-0.5), emphasizing the detrimental effect of malnutrition on life expectancy.

- Variables like Hepatitis B Immunization Coverage (0.2), Polio Immunization Coverage (0.31), DTP3 Immunization Coverage (0.31), and Health Expenditure (0.4 for GDP % and 0.22 for total expenditure) all show positive correlations, suggesting that better healthcare access and immunization rates contribute positively to life expectancy.

- Human Development Index (HDI) (0.76) and Years of Schooling (0.77) have the strongest positive correlations with life expectancy, underlining the significant role of socio-economic development and education in improving health outcomes.

- There is a statistically significant positive association between GDP and life expectancy, with a p-value of less than 2e-16. The correlation coefficient of 0.4615 indicates a moderately strong relationship, suggesting that as GDP increases, life expectancy tends to rise, though the strength of the relationship is not overwhelming. The linear regression model explains 21.29% of the variation in life expectancy (R-squared = 0.2129), indicating that while GDP plays a role in life expectancy, other factors are also crucial. Further research incorporating a broader range of variables would provide deeper insights into the determinants of life expectancy.

- There is a statistically significant relationship between schooling and life expectancy, evidenced by the very low p-value from the regression model. Specifically, each additional year of schooling is associated with an expected increase of approximately 2.32 years in life expectancy. Moreover, schooling accounts for about 61% of the variance in life expectancy, highlighting its substantial impact.

- Statistical analyses strongly reject the null hypothesis, indicating a significant difference in life expectancy between developed and developing countries, with developed nations averaging 12.27 years longer. While correlation and regression analyses show a positive relationship between development status and life expectancy, residual and Q-Q plots suggest other influencing factors may be unaccounted for. Boxplots further reveal that developed countries have a higher median and smaller spread in life expectancy. Thus, development status significantly predicts life expectancy, but further exploration of additional variables is necessary for a more comprehensive understanding.

- A Pearson correlation analysis revealed a moderate positive correlation (r = 0.405) between alcohol consumption (mean: 4.61 litres per capita) and life expectancy (mean: 69.17 years), indicating that higher alcohol consumption is associated with increased life expectancy. However, with approximately 16.39% of the variance in life expectancy explained by this relationship, it is essential to note that correlation does not imply causation. Residual analysis indicates factors such as linearity, normality, and homoscedasticity, along with other influencing variables, suggest that alcohol consumption alone is not a strong predictor of life expectancy, emphasizing the role of additional factors in determining life outcomes.

- The multiple linear regression model demonstrates strong performance, with an R-squared of 0.8202, indicating that 82.02% of the variance in life expectancy is explained by the predictors. All coefficients are statistically significant (p < 0.001), and the residual standard error of 3.972 suggests predictions closely align with observed values. Key findings include positive impacts on life expectancy from being in a developed country, increased health expenditure, and improved vaccination rates, while adult mortality, measles cases, and infant deaths due to HIV/AIDS negatively affect life expectancy. The model's intercept of 51.96 years serves as a baseline for analysis.

# References

1. Kumar, A. (2020). Life Expectancy (WHO) [Data set]. Kaggle. https://www.kaggle.com/datasets/kumarajarshi/life-expectancy-who
